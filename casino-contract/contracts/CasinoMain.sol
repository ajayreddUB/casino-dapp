...
contract CasinoItems is ERC1155 {
    ...
    constructor(
        address payable _casinoChipAddress,
        address _casinoNFTAddress
    ) ERC1155("Casino1155") {
        owner = msg.sender;
    ...
    function approveCasinoChip(uint256 amount) external {
        casinoChip.Approve(msg.sender, address(this), amount);
    }

    function approveCasinoNFT(address ownerNFT) external {
        casinoNFT._approveContract(ownerNFT, address(this));
    }
    ...
    function uri(uint256 tokenId) public view override returns (string memory) {
        if (tokenId >= NFT_ID) {
            // Assuming NFT_ID and upwards are for ERC-721 tokens
    ...
    function buyNFTWithERC20(uint256 tokenId, uint256 erc20Amount) public {
        ...
        require(
            erc20Amount >=
                (nftPrice / casinoChip.tokenPrice()) *
                    10 ** casinoChip.decimals(),
            "Not enough ERC20 tokens to buy the NFT"
        );
    ...
    function rollDice(
        ...
        require(
            casinoChip.balanceOf(msg.sender) >= betAmount,
            "Insufficient CasinoChip balance"
        );
        ...
        seed = uint256(
            keccak256(abi.encodePacked(seed, userProvidedSeed, msg.sender))
        );
        seed =
            uint(
        ...
        if (winningNumber == chosenNumber) {
            ...
            require(
                casinoChip.balanceOf(casinoChip.owner()) >= reward,
                "Insufficient CasinoChip in owner"
            );
    ...
    function customSafeBatchTransferFrom(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            if (id == CHIP_ID) {
                ...
                require(
                    casinoChip.balanceOf(from) >= amount,
                ...
                require(
                    amount == 1,
                    "Can only transfer one ERC-721 token at a time"
                );
                require(
                    casinoNFT.ownerOf(id) == from,
                    "ERC-721 token not owned by sender"
                );
                casinoNFT.safeTransferFrom(from, to, id);
            } else {
